...
def test_estimate_vsi_direct_plating_production_compact_rectangle(monkeypatch, tmp_path):
    ...
    assert estimate["pyramid_generation_policy"] == "native_source_pyramid_crop"
    assert estimate["source_tile_aligned_canvas"] is True
    assert estimate["native_mip_stop_level_source"] == "segmentation_level"
    assert estimate["context_margin_px"] == 0
    ...
    assert (
        tissue["combined_logical_chunks"]
        == tissue["all_mip_chunks"] + tissue["mask_all_mip_chunks"]
    )
    ...
    assert (
        totals["combined_logical_chunks"]
        == totals["all_mip_chunks"] + totals["mask_all_mip_chunks"]
    )
...
def test_estimate_vsi_direct_plating_native_stops_at_segmentation_level(monkeypatch, tmp_path):
    from wsi_pipeline.config import TileConfig
    from wsi_pipeline.pipeline import vsi_ets
    ...
    class _DummyETS:
        ...
        def level_shape(self, level):
            return [(1024, 1024), (512, 512), (256, 256), (128, 128), (64, 64)][level]
    ...
    tissue = estimate["tissues"][0]
    assert tissue["output_scale_to_source_level"] == {"s0": 0, "s1": 1, "s2": 2, "s3": 3}
    assert tissue["mip_stop_reason"] == "segmentation_level"
    assert tissue["per_scale_shapes_yx"] == [[512, 512], [256, 256], [128, 128], [64, 64]]
    assert tissue["source_tile_alignment_expansion_yx"] == {
        "top": 0,
        "bottom": 0,
        "left": 0,
        "right": 0,
    }
    assert estimate["projection"]["baseline"]["elapsed_s"] == 228.3
    assert estimate["projection"]["projected_elapsed_s"] is not None
    assert estimate["totals"]["projection_baseline_logical_chunks"] > 0
    assert any(
        warning.startswith("baseline_config_mismatch:segmentation_level")
        for warning in estimate["totals"]["warnings"]
    )
...
def test_e241_production_config_uses_masked_native_zero_margin():
    from wsi_pipeline.config import load_config
    from wsi_pipeline.pipeline import vsi_ets

    config = load_config(Path("configs/e241_production_default.yaml"))
    defaults = vsi_ets._profile_defaults("production")

    assert defaults["extra_margin_px"] == 0
    ...
    assert config.output.masked_rgb_fill_value == 0
...
def test_e241_debug_context_config_preserves_margin_64():
    from wsi_pipeline.config import load_config

    config = load_config(Path("configs/e241_debug_context.yaml"))

    assert config.tiles.extra_margin_px == 64
    assert config.output.primary_rgb_mode == "unmasked_rgb"
    assert config.output.pyramid_generation_policy == "native_source_pyramid_crop"


def test_source_level_0_runbook_is_manual_and_uses_glob_outputs():
    runbook = Path("docs/source_level_0_production_runbook.md").read_text(encoding="utf-8")

    assert "--source-level 0" in runbook
    assert "manual" in runbook.lower()
    assert "resume=True" in runbook
    assert 'find "$BASE/per_tissue_ngff" -maxdepth 1 -name "*.ome.zarr"' in runbook
    assert "process_vsi_directory_with_plating" in runbook
    assert "source_level=0" in runbook
...
def test_process_vsi_directory_with_plating_reuses_source_and_returns_mapping(
    ...
    def _fake_vsi_to_source_ome_zarr(vsi_path, output_path, **kwargs):
        ...
        return (
            Path(output_path),
            Path(f"/ets/{Path(vsi_path).stem}.ets"),
            {"physical_pixel_size_um": {"x": 0.25, "y": 0.5, "z": None}},
        )
    ...
    assert (
        "expected_tissues"
        not in inspect.signature(vsi_ets.process_vsi_directory_with_plating).parameters
    )
...
def test_process_vsi_directory_with_plating_uses_direct_ets_by_default(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: pytest.fail(
            "default directory path should not materialize source OME-Zarr"
        ),
    )
    ...
    assert results == {
        str(vsi_path): [tmp_path / "out" / "per_tissue_ngff" / "a_tissue_00.ome.zarr"]
    }
...
def test_direct_vsi_plating_forwards_uncompressed_streaming_progress(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(vsi_ets, "ETSFile", _DummyETS)
    monkeypatch.setattr(
        vsi_ets, "_segment_for_plating", lambda *args, **kwargs: (np.ones((8, 8), dtype=bool), {})
    )
...
def test_write_native_ets_tissue_pyramid_writes_native_levels_and_metrics(monkeypatch, tmp_path):
    ...
    assert root.attrs["native_source_pyramid"]["canonical_canvas_in_source_level_coordinates"] == {
        "y0": 0,
        "x0": 0,
        "y1": 8,
        "x1": 8,
